...
form.addEventListener("submit", async (event) => {
  ...
  const dietary_restrictions =
      document.querySelector('#dietary-restrictions').value;

  const prompt =
      `You are a cooking chatbot that can help me prepare the perfect meal! Given the expertise level of ${
          expertise}, complexity of ${complexity}, budget of ${
          budget}, preparation time of ${time} minutes, serving scale of ${
          scale}, nutrition preference of ${nutrition}, taste preference of ${
          taste}, dietary restrictions of ${
          dietary_restrictions}, and available ingredients of ${
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      headers : {
        ...
        'Authorization' :
            'Bearer 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' // Replace
                                                                         // with
                                                                         // your
                                                                         // OpenAI
                                                                         // API
...
function displayRecipe(data) {
  ...
  const message = data.choices[0].message.content.trim().replace(/\n/g, "<br>");
  recipeElement.innerHTML =
      `<div class="message"><div class="bot-message">${message}</div></div>`;
